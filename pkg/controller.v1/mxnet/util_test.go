...
func TestGenOwnerReference(t *testing.T) {
	...
	if ref.APIVersion != mxv1.SchemeGroupVersion.String() {
		t.Errorf("Expected APIVersion %s, got %s", mxv1.SchemeGroupVersion.String(), ref.APIVersion)
	}
}
...
func TestGenLabels(t *testing.T) {
	...
	if labels[labelGroupName] != mxv1.GroupName {
		t.Errorf("Expected %s %s, got %s", labelGroupName, mxv1.GroupName, labels[labelGroupName])
	}
}
