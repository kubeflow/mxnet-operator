...
func TestCopyLabelsAndAnnotation(t *testing.T) {
	...
	mxJob.Spec.MXReplicaSpecs[mxv1.MXReplicaTypeWorker].Template.Labels = labels
	mxJob.Spec.MXReplicaSpecs[mxv1.MXReplicaTypeWorker].Template.Annotations = annotations
	unstructured, err := testutil.ConvertMXJobToUnstructured(mxJob)
	if err != nil {
		t.Errorf("Failed to convert the MXJob to Unstructured: %v", err)
	}
...
func TestDeletePodsAndServices(t *testing.T) {
	...
	testCases := []testCase{
		testCase{
			...
			mxJob:       testutil.NewMXJobWithCleanPolicy(1, 4, 2, mxv1.CleanPodPolicyAll),
		...
		testCase{
			...
			mxJob:       testutil.NewMXJobWithCleanPolicy(1, 4, 2, mxv1.CleanPodPolicyRunning),
		...
		testCase{
			...
			mxJob:       testutil.NewMXJobWithCleanPolicy(1, 4, 2, mxv1.CleanPodPolicyNone),
	...
	for _, tc := range testCases {
		...
		err := updateMXJobConditions(tc.mxJob, mxv1.MXJobSucceeded, mxJobSucceededReason, "")
		if err != nil {
