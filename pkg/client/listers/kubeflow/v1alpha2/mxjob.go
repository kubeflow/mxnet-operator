...
type MXJobLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha2.MXJob, err error)
...
// List lists all MXJobs in the indexer.
func (s *mXJobLister) List(selector labels.Selector) (ret []*v1alpha2.MXJob, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha2.MXJob))
	})
	return ret, err
}
...
type MXJobNamespaceLister interface {
	...
	Get(name string) (*v1alpha2.MXJob, error)
...
// List lists all MXJobs in the indexer for a given namespace.
func (s mXJobNamespaceLister) List(selector labels.Selector) (ret []*v1alpha2.MXJob, err error) {
...
// Get retrieves the MXJob from the indexer for a given namespace and name.
func (s mXJobNamespaceLister) Get(name string) (*v1alpha2.MXJob, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha2.Resource("mxjob"), name)
	}
	return obj.(*v1alpha2.MXJob), nil
}
