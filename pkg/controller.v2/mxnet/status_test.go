...
func TestFailed(t *testing.T) {
	...
	err := updateStatusSingle(mxJob, mxv1alpha2.MXReplicaTypeWorker, 3, false, false)
	if err != nil {
...
func TestStatus(t *testing.T) {
	...
	for i, c := range testCases {
		...
		if _, ok := c.mxJob.Spec.MXReplicaSpecs[mxv1alpha2.MXReplicaTypeScheduler]; ok {
			err := updateStatusSingle(c.mxJob, mxv1alpha2.MXReplicaTypeScheduler, 1, c.restart, c.schedulerCompleted)
			if err != nil {
			...
			if c.mxJob.Spec.MXReplicaSpecs[mxv1alpha2.MXReplicaTypeWorker] != nil {
				...
				err := updateStatusSingle(c.mxJob, mxv1alpha2.MXReplicaTypeWorker, int(*replicas), c.restart, c.schedulerCompleted)
				if err != nil {
					t.Errorf("%s: Expected error %v to be nil", c.description, err)
				}
			}
			if c.mxJob.Spec.MXReplicaSpecs[mxv1alpha2.MXReplicaTypeServer] != nil {
				...
				err := updateStatusSingle(c.mxJob, mxv1alpha2.MXReplicaTypeServer, int(*replicas), c.restart, c.schedulerCompleted)
				if err != nil {
					t.Errorf("%s: Expected error %v to be nil", c.description, err)
				}
			}
