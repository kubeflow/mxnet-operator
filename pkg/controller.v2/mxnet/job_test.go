...
func TestAddMXJob(t *testing.T) {
	...
	ctr.deleteMXJobHandler = func(mxjob *mxv1alpha2.MXJob) error {
		return nil
	}
...
func TestCopyLabelsAndAnnotation(t *testing.T) {
	...
	mxJob.Spec.MXReplicaSpecs[mxv1alpha2.MXReplicaTypeWorker].Template.Labels = labels
	mxJob.Spec.MXReplicaSpecs[mxv1alpha2.MXReplicaTypeWorker].Template.Annotations = annotations
	unstructured, err := testutil.ConvertMXJobToUnstructured(mxJob)
	if err != nil {
		t.Errorf("Failed to convert the MXJob to Unstructured: %v", err)
	}
...
func TestDeletePodsAndServices(t *testing.T) {
	...
	testCases := []testCase{
		testCase{
			...
			mxJob:       testutil.NewMXJobWithCleanPolicy(1, 4, 2, mxv1alpha2.CleanPodPolicyAll),
	...
	for _, tc := range testCases {
		...
		err := updateMXJobConditions(tc.mxJob, mxv1alpha2.MXJobSucceeded, mxJobSucceededReason, "")
		if err != nil {
